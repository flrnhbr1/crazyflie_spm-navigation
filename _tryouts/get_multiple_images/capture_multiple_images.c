...
int test_camera()
{
    ...
    for(int i = 0; i < bufferCount; i++){
        ...
        if(buff[i] == NULL){return -1;}
    }
    //buff = pmsis_l2_malloc(BUFF_SIZE);
    //if (buff == NULL){ return -1;}
    printf("Initialized buffer\n");
    int n;
    // printf("Type amount of pictures to capture:\n");
    // scanf("%d", &n);
    n = bufferCount;
    int offset = 42;
    for (int i = 0; i < n; i++){
        ...
        printf("1\n");
        printf("0\n");
        printf("taking capture %d\n", i);

        #ifdef ASYNC_CAPTURE
            // Start up async capture task
            done = 0;
            pi_task_t task;
            pi_camera_capture_async(&camera, buff[i], BUFF_SIZE, pi_task_callback(&task, handle_transfer_end, NULL));
        #endif
        ...
        pi_camera_control(&camera, PI_CAMERA_CMD_START, 0);
            ...
            while(!done){pi_yield();}
        #else
            pi_camera_capture(&camera, buff[i], BUFF_SIZE);
        #endif
    ...
    char filepath[6];
    for(int i = 0; i < bufferCount; i++){
        sprintf(filepath, "../../../images/%d.ppm", i+offset);
        WriteImageToFile(filepath, WIDTH, HEIGHT, sizeof(uint8_t), buff[i], GRAY_SCALE_IO );
    }
